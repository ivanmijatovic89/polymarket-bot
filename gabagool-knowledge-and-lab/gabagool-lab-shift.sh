#!/usr/bin/env bash
# Gabagool lab shift: an autonomous, greenfield research LAB for exactly ONE
# strategy concept — the "gabagool" two-sided pair accumulation on Polymarket
# BTC 15m up/down markets.
#
# Unlike the knowledge shift (research only, no backtests), this lab DESIGNS
# ITS OWN research system — evaluation, champion scoring, experiment policy,
# memory, tools — taking whatever it wants from strategy-research-protocol/
# and fable-lab/ without being bound by either. Then it builds strategy
# variants, backtests them, and drives toward a live-ready validated variant
# or a numeric proof that the concept can't pay on this book.
#
# Pattern: clone of scripts/fable-night-shift.sh (proven over 69 sessions).
# Branch `gabagool-lab`, worktree ../polymarket-bot-gabagool-lab, writes ONLY
# inside `gabagool-lab/` and `src/strategies/gabagool-lab/` (pre-commit hook).
#
# Usage (from the MAIN checkout, not the worktree):
#   ./gabagool-knowledge-and-lab/gabagool-lab-shift.sh              # start / resume
#   MAX_RUNS=10 ./gabagool-knowledge-and-lab/gabagool-lab-shift.sh
#   MODEL=fable ./gabagool-knowledge-and-lab/gabagool-lab-shift.sh
#
# Account: the loop uses whatever `claude` profile this shell has. Pass
# CLAUDE_CONFIG_DIR to choose the account per launch, e.g.:
#   CLAUDE_CONFIG_DIR=$HOME/.claude-balsa MODEL=fable ./gabagool-knowledge-and-lab/gabagool-lab-shift.sh
#
# Overnight (recommended):
#   tmux new -s gabalab
#   caffeinate -is ./gabagool-knowledge-and-lab/gabagool-lab-shift.sh
#
# Stop: touch ../polymarket-bot-gabagool-lab/gabagool-lab/DONE   (or Ctrl+C)
# The lab may create DONE itself ONLY at mission end as the charter defines.
#
# Rate-limit resilience: runs shorter than MIN_RUN_SECS are treated as failed
# launches (token exhaustion) and the loop backs off FAIL_SLEEP seconds, so it
# survives weekly-limit windows and resumes when credits reset.
#
# Morning review:
...
BRANCH="gabagool-lab"
WT="${ROOT}/../polymarket-bot-gabagool-lab"
LAB="gabagool-lab"
...
# ---------------------------------------------------------------- setup ----
git fetch origin main --quiet 2>/dev/null || echo "[lab-shift] fetch failed (offline?) — using local refs"

if ! git show-ref --verify --quiet "refs/heads/${BRANCH}"; then
  BASE="main"
  git show-ref --verify --quiet "refs/remotes/origin/main" && BASE="origin/main"
  git branch "${BRANCH}" "${BASE}"
  echo "[lab-shift] created branch ${BRANCH} from ${BASE}"
fi

if [ ! -d "$WT" ]; then
  git worktree add "$WT" "${BRANCH}"
  echo "[lab-shift] created worktree ${WT}"
fi
...
2. `gabagool-knowledge-and-lab/INVESTIGATION.md`,
   `gabagool-knowledge-and-lab/MISSION.md`,
   `gabagool-knowledge-and-lab/PLAYBOOK.md`.
...
CHARTER
  ...
  git -C "$WT" commit -m "gabagool-lab: shift charter + write-scope guard" >/dev/null
  git -C "$WT" push -u origin "${BRANCH}" 2>/dev/null || echo "[lab-shift] push failed (offline?) — continuing, session will retry"
  echo "[lab-shift] charter committed"
fi
...
directory. Resume from gabagool-lab/STATE.md (if it does not exist yet, this \
is the first session: create it, then start Phase 0). Work continuously and \
autonomously; never ask questions; commit and push after every unit of work. \
Create gabagool-lab/DONE only at mission end (L3) as the charter defines; \
...
while [ "$run" -lt "$MAX_RUNS" ]; do
  if [ -f "${WT}/${LAB}/DONE" ]; then
    echo "[lab-shift] DONE marker found — stopping."
    break
  fi
  run=$((run + 1))
  LOG="${WT}/${LAB}/logs/shift-$(date +%Y%m%d-%H%M%S).jsonl"
  echo "[lab-shift] run ${run}/${MAX_RUNS} — log: ${LOG}"
  RUN_STARTED=$(date +%s)
  ...
  [ -f "${WT}/${LAB}/DONE" ] && { echo "[lab-shift] DONE — stopping."; break; }

  RUN_SECS=$(( $(date +%s) - RUN_STARTED ))
  if [ "$RUN_SECS" -lt "$MIN_RUN_SECS" ]; then
    echo "[lab-shift] run lasted only ${RUN_SECS}s — likely rate limit / launch failure; backing off ${FAIL_SLEEP}s"
    sleep "$FAIL_SLEEP"
  else
    echo "[lab-shift] relaunching in ${SLEEP_BETWEEN}s (Ctrl+C to stop)"
    sleep "$SLEEP_BETWEEN"
  fi
done

echo "[lab-shift] finished after ${run} run(s) — total cost this invocation: \$${TOTAL_COST:-0} (API-equivalent)"
echo "[lab-shift] review: cd ${WT} && git log --oneline main..${BRANCH}"
