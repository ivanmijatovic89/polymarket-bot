...
| `parity.ts` | per-market row parity between two runs of the same spec across 18 deterministic columns (D36 fleet/local check; outcome-safe — values print only on mismatch; `--intersection` for superset runs; exit 2 = divergence) |
