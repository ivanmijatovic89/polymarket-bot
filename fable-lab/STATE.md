...
**Session 67 — FINAL RUN v2 (operator directive in the 2026-07-12
operator update below).** Progress this session, in order: (1) BATCH-005
JUDGED and ledgered — 0 survive / 19 kill / 1 park-design, table +
integrity checks in BATCH-005.md (U106); (2) fresh-context batch checker
SOUND-WITH-FINDINGS, 3 MINOR all applied
(`knowledge/AUDIT-2026-07-12-BATCH-005-CHECKER.md`, U107);
(3) propagation done — LESSONS E31 (exit axis measured and lost
everywhere tested), IDEAS 42/41 dead/#10 parked, EDGE-SPACE map row +
exit-axis bullet + §4 exit bar, INDEX regenerated (U108); (4) the
RESCUE-025 program FROZEN pre-sweep (U109,
`protocol/registry/screens/RESCUE-025.md`): 40-variant grid, mechanical
winner rule (highest t among q̂>0 ∧ played≥100, needs t≥+1.5 else dead
at sweep), anti-curse confirmation pre-committed (seeded 4,000-market
reserve draw, seed RESCUE-025-draw-1, sha b77ba0cb…, bar q̂>0 ∧ t≥+1.5,
ONE shot → SAVED or DEAD FOR GOOD); (5) smokes green incl. exact
baseline equivalence vs fable-scr-stm (U110); (6) **SWEEP IN FLIGHT:**
40 fleet runs RSC-025-V01..V40 submitted --detach (U111, submit log
`logs/rsc025-submit.log`, latency pinned via export per D8).

If this session dies mid-flight: wait for the 40 RSC-025-Vnn runs, then
(a) window-integrity check (every drawn market_start_ms <
1772323200000), (b) E28 parity check on ONE sweep run (12 markets local
pinned re-run, byte-compare — the fleet self-updated to NEW code for
this batch), (c) apply the FROZEN winner rule in RESCUE-025.md §winner
mechanically, (d) if a winner exists: submit the confirmation
(8 fleet runs, `--slug` per `logs/RESCUE-025-shard[0-7].slugs`
regenerated by `tools/rescue-draw.ts` — MUST reproduce the sha in the
spec — batchUids RSC-025-CONFIRM-S0..S7, winner's exact params), pooled
read per scr009-pool pattern, judge on the frozen bar; (e) fresh-context
checker over the whole program verdict; (f) terminal handoff (SAVED
candidate write-up or DEAD FOR GOOD) — the operator says this is the
last session for months; leave STATE as a walk-away document like the
session-65 closing sprint did.
