...
_Last updated: session 40, unit U43az._
...
- U43c: CAL-001 registration AUDITED pre-results by a fresh-context
  ...
  re-armed on pid exit.
  Session 24 check (~58 min in): 4,244/8,516 (~0.82s/market, ETA
  ~58 min); latency 0/0, UP/DOWN exact balance (24,419 each), 0 error
  lines, epoch range [1764460800, 1768467600] inside the discovery
  window, 8,516 files loaded, 34 GiB free disk. Mirror re-measurement
  on the grown log (4-decimal rounding per the session-22 method note):
  24,495/24,496 paired (market,offset) samples exactly DOWN = 1 − UP;
  the SAME single deviant (1764846000, off=850) — amendment #12 holds
  at ~50% of the run. Asset-pairing completeness re-confirmed (0
  one-sided keys). NEW outcome-free check: ts monotonicity — within
  every (market,asset) series, sample ts is non-decreasing across
  ...
  Session 26 check (~71.5 min in): 4,919/8,516 (~0.87s/market, ETA
  ~52 min); latency 0/0, UP/DOWN exact balance (28,353 each), 0 error
  lines, epoch range [1764460800, 1769082300] inside the discovery
  window, 8,516 files loaded, 34 GiB free disk. Mirror re-measurement
  on the grown log (4-decimal rounding): 28,414/28,415 paired
  (market,offset) samples exactly DOWN = 1 − UP; the SAME single
  ...
  Session 28 check (~112 min in): 6,275/8,516 (ETA ~40 min); full D23
  integrity battery via `tools/calib-integrity.sh` GREEN in one shot —
  latency 0/0, 0 error lines, progress gaps=0 dupfiles=0, 74,928 sample
  lines 0 malformed, UP/DOWN exact balance (37,464 each), epoch max
  1770305400 < frozen bound with 0 slug/epoch mismatches, fields clean
  (badoff=0, tsbounds=0, crossed=0), dedupe clean (0 dup tuples, 0
  markets over the 14-line cap), mirror 37,464 paired / the SAME single
  known deviant (1764846000/850) — amendment #12 holds at ~74% of the
  run — 0 one-sided keys, ts monotonicity 0 violations. Coverage via
  frozen calib-coverage.sh at 5,896 slugs: 600s→0.9678, 750s→0.8397,
  ...
  Session 30 check (~117 min in): 6,393/8,516 (ETA ~39 min); full D23
  integrity battery via `tools/calib-integrity.sh` GREEN in one shot —
  latency 0/0, 0 error lines, gaps=0 dupfiles=0, 76,522 sample lines
  0 malformed, UP/DOWN exact balance (38,261 each), epoch max
  1770412500 < frozen bound with 0 mismatches, fields clean, dedupe
  clean, mirror 38,261 paired / the SAME single known deviant
  (1764846000/850) — 0 one-sided keys, ts monotonicity 0 violations.
  Coverage via frozen calib-coverage.sh at 6,013 slugs: 600s→0.9684,
  750s→0.8428, 850s→0.5651. 34 GiB free disk. Tool provenance
  re-verified: tree clean, calib.ts still @ ab2acc9, calib-coverage.sh
  @ 639ad8c. Background watch armed on pid 73037 exit.
  Session 31 check (~120 min in): 6,459/8,516 (ETA ~38 min); full D23
  integrity battery via `tools/calib-integrity.sh` GREEN in one shot —
  latency 0/0, 0 error lines, gaps=0 dupfiles=0, 77,414 sample lines
  0 malformed, UP/DOWN exact balance (38,707 each), epoch max
  1770471900 < frozen bound with 0 mismatches, fields clean, dedupe
  clean, mirror 38,707 paired / the SAME single known deviant
  (1764846000/850) — 0 one-sided keys, ts monotonicity 0 violations.
  Coverage via frozen calib-coverage.sh at 6,081 slugs: 600s→0.9688,
  750s→0.8446, 850s→0.5680. 33 GiB free disk. Background watch armed
  on pid 73037 exit.
  Session 32 check (~137 min in): 6,943/8,516 (ETA ~31 min); full D23
  integrity battery via `tools/calib-integrity.sh` GREEN in one shot —
  latency 0/0, 0 error lines, gaps=0 dupfiles=0, 83,966 sample lines
  0 malformed, UP/DOWN exact balance (41,983 each), epoch max
  1770907500 < frozen bound with 0 mismatches, fields clean, dedupe
  clean, mirror 41,983 paired / the SAME single known deviant
  (1764846000/850) — 0 one-sided keys, ts monotonicity 0 violations.
  Coverage via frozen calib-coverage.sh at 6,562 slugs: 600s→0.9712,
  750s→0.8557, 850s→0.5826. 34 GiB free disk. Tool provenance
  ...
  Session 34 check (~142 min in): 7,102/8,516 (ETA ~28 min); full D23
  integrity battery via `tools/calib-integrity.sh` GREEN in one shot —
  latency 0/0, 0 error lines, gaps=0 dupfiles=0, 86,128 sample lines
  0 malformed, UP/DOWN exact balance (43,064 each), epoch max
  1771050600 < frozen bound with 0 mismatches, fields clean, dedupe
  clean, mirror 43,064 paired / the SAME single known deviant
  (1764846000/850) — 0 one-sided keys, ts monotonicity 0 violations.
  Coverage via frozen calib-coverage.sh at 6,728 slugs: 600s→0.9718,
  750s→0.8591, 850s→0.5878 — late-offset drift continues; final-log
  fractions bind for amendment #11 wording. 33 GiB free disk. Watch
  armed on pid 73037 exit (60-min window covers the ETA).
  Session 35 check (~145 min in): 7,206/8,516 (ETA ~26 min at
  ~1.2s/market); full D23 integrity battery via
  `tools/calib-integrity.sh` GREEN in one shot — latency 0/0, 0 error
  lines, gaps=0 dupfiles=0, 87,554 sample lines 0 malformed, UP/DOWN
  exact balance (43,777 each), epoch max 1771144200 < frozen bound with
  0 mismatches, fields clean, dedupe clean, mirror 43,777 paired / the
  ...
  6,833 slugs: 600s→0.9723, 750s→0.8614, 850s→0.5924 — late-offset
  ...
  background watch; ETA inside this session's window — the verdict may
  land THIS session).
  Session 37 check (~150 min in): 7,389/8,516 (ETA ~23 min at
  ~1.2s/market); full D23 integrity battery via
  `tools/calib-integrity.sh` GREEN in one shot — latency 0/0, 0 error
  lines, gaps=0 dupfiles=0, 90,026 sample lines 0 malformed, UP/DOWN
  exact balance (45,013 each), epoch max 1771308900 < frozen bound with
  0 mismatches, fields clean, dedupe clean, mirror 45,013 paired / the
  SAME single known deviant (1764846000/850) — 0 one-sided keys, ts
  monotonicity 0 violations. Coverage via frozen calib-coverage.sh at
  7,016 slugs: 600s→0.9729, 750s→0.8643, 850s→0.5965 — late-offset
  drift continues; final-log fractions bind for amendment #11 wording.
  34 GiB free disk. Watch armed on pid 73037 exit (in-session
  background watch; ETA inside this session's window — the verdict may
  land THIS session).
  Session 38 check (~152 min in): 7,473/8,516 (ETA ~20 min at
  ~1.2s/market); full D23 integrity battery via
  `tools/calib-integrity.sh` GREEN in one shot — latency 0/0, 0 error
  lines, gaps=0 dupfiles=0, 91,154 sample lines 0 malformed, UP/DOWN
  exact balance (45,577 each), epoch max 1771385400 < frozen bound with
  0 mismatches, fields clean, dedupe clean, mirror 45,577 paired / the
  SAME single known deviant (1764846000/850) — 0 one-sided keys, ts
  monotonicity 0 violations. Coverage via frozen calib-coverage.sh at
  7,101 slugs: 600s→0.9732, 750s→0.8655, 850s→0.5981 — late-offset
  drift continues; final-log fractions bind for amendment #11 wording.
  ...
  background watch; ETA inside this session's window — the verdict may
  land THIS session).
  Session 40 check (~158 min in): 7,699/8,516 (ETA ~16 min); full D23
  integrity battery via `tools/calib-integrity.sh` GREEN in one shot —
  latency 0/0, 0 error lines, gaps=0 dupfiles=0, 94,154 sample lines
  0 malformed, UP/DOWN exact balance (47,077 each), epoch max
  1771587900 < frozen bound with 0 mismatches, fields clean, dedupe
  clean, mirror 47,077 paired / the SAME single known deviant
  (1764846000/850) — 0 one-sided keys, ts monotonicity 0 violations.
  Coverage via frozen calib-coverage.sh at 7,320 slugs: 600s→0.9740,
  750s→0.8690, 850s→0.5997 — late-offset drift continues; final-log
  fractions bind for amendment #11 wording. 34 GiB free disk. Watch
  armed on pid 73037 exit (in-session background watch; ETA inside
  this session's window — the verdict may land THIS session).
