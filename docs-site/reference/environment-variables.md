...
| `POLYGON_RPC_URL`        | `string`                   | `https://polygon-bor-rpc.publicnode.com`               | Polygon JSON-RPC endpoint used for on-chain operations (splits, merges, redeems).  |
...
| Variable                                   | Type                             | Default       | Description                                                                                                                                                                                                                                   |
| ------------------------------------------ | -------------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DRY_RUN`                                  | `boolean`                        | `true`        | When `true` (or any value other than the string `false`), orders are validated and logged but never submitted to the CLOB. Set to `false` to enable real order placement.                                                                     |
...
| `USER_WS_FILL_AT_STATUS`                   | `MATCHED \| MINED`               | _(empty)_     | Controls which status event triggers a fill update in the Portfolio. `MATCHED` is faster; `MINED` is required before selling or merging shares received in a buy. See [Fill-status semantics](/live-trading/live-trading-bot#fill-status-semantics). |
...
| `BACKTEST_TAKER_FEE_BPS`                 | `integer (bps)` | `156`     | Taker fee rate in basis points applied by the backtest execution simulator. Must be a non-negative integer; invalid values fall back to `156`.                                                   |
