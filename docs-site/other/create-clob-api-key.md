...
When running multiple bots with different wallets, place each wallet's credentials in its own `.env.botX` file and set `BOT_ENV=botX` before starting that bot instance. See [Multiple Bots](./MultipleBots.md) for the full setup.
:::
