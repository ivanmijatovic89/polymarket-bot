...
| `USER_WS_FILL_AT_STATUS`                   | `MATCHED \| MINED`               | _(empty)_     | Controls which status event triggers a fill update in the Portfolio. `MATCHED` is faster; `MINED` is required before selling or merging shares received in a buy. See [Fill-status semantics](/other/live-trading-bot#fill-status-semantics). |
