...
External feeds are **strategy-driven**: a strategy opts in by registering `ExternalFeedsRequestPlugin` (or via legacy `strategy.requiredFeeds`). Live, `trading-bot.ts` only starts feed clients requested by the selected strategy. In **backtests**, the `binanceWsSpotPrice` sub-feed is fulfilled automatically from historical `data.binance.vision` aggTrades (as-of lookup, measured-latency offset; missing day files are a hard error — see `docs/datasets/polymarket-data/binance-aggtrades-feed.md` and `npm run binance:download-aggtrades`). The other sub-feeds (`rtdsPolymarketCryptoPrices`, `polymarketPriceToBeat`, `deribitVolatilityIndex`) are still live-only.
