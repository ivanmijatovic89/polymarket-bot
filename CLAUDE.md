...
- **Backtest extension (`--extend <runId>`)**: appends new markets to an existing `backtest_runs` row and recomputes `batch_stats` + `chunked_batch_stats` over the union. **No new tables**. The parent's `batch_uid` is overwritten with a `-extN` suffix on each extend; old `batch_uid` values are not retained. `cmd` on the parent row is overwritten with the extend invocation. Don't run two `--extend <same id>` invocations in parallel — there's no app-level lock; the DB transaction's `SELECT FOR UPDATE` serialises the merge step but candidate-set planning can race and produce duplicate-idx errors. Strategy/params/symbol/timeframe/converter/readFrom always inherit from the parent; pass them and you get a clear error.
