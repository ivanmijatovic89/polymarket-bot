...
function fmtSide(side: 'UP' | 'DOWN', s: DwellGateSide | undefined): string {
  const elapsedMs =
    typeof s?.elapsedInRangeMs === 'number' && Number.isFinite(s.elapsedInRangeMs)
      ? s.elapsedInRangeMs
      : null
  const remainingMs =
    typeof s?.remainingMs === 'number' && Number.isFinite(s.remainingMs) ? s.remainingMs : null
...
function buildTitle(d: DwellGateSnapshot | undefined): string {
  ...
  const requiredMs =
    typeof d?.requiredMs === 'number' && Number.isFinite(d.requiredMs) ? d.requiredMs : null
