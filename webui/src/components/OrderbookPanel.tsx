...
export function OrderbookPanel(props: { label: 'UP' | 'DOWN'; book?: BotUiOrderBook; compact?: boolean }) {
  ...
  return (
    <div className={`panel ${compact ? 'p-2.5' : 'p-3'}`}>
      <div className={`flex items-center justify-between gap-2 ${compact ? 'mb-1.5' : 'mb-2'}`}>
        <div className={`${compact ? 'text-[15px]' : 'text-[17px]'} font-semibold`}>{props.label}</div>
        <div className={`flex items-center gap-2 font-mono ${compact ? 'text-[14px]' : 'text-[16px]'} text-zinc-300`}>
      ...
      {book ? (
        <div className={compact ? 'space-y-1.5' : 'space-y-2'}>
          ...
          <div className={`rounded-md bg-zinc-900/40 ring-1 ring-zinc-800 ${compact ? 'px-2 py-1.5' : 'px-3 py-2'}`}>
            <div className={`flex items-center justify-between gap-2 ${compact ? 'text-[14px]' : 'text-[16px]'}`}>
