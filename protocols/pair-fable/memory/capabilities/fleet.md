...
- Provenance lands: backtest_runs.protocol='pair-fable', model='claude-fable-5', cmd = full argv incl. latency flags + injected --batchUid. [db run 854/855 | 2026-07-30]
...
`tsx protocols/pair-fable/tools/fleet.ts [--json]` — queue counts (both queues, incl. waiting-children), worker liveness, active aggregate batches with processed/unprocessed child counts. Verified against a live batch (caught mid/post-run worker state; batches < ~20s finish faster than polling). Patterns inside:
