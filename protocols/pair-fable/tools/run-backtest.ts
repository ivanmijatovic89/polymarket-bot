/**
 * run-backtest.ts — the canonical pair-fable backtest launcher.
 *
 * Usage (from repo root):
 *   tsx protocols/pair-fable/tools/run-backtest.ts --strategy <id> [options]
 *
 ...
 *      140/20 ms, provenance --protocol pair-fable --model <id>.
 ...
 *      latency (PROPOSALS P-001), and every pair-fable run is latency-pinned
...
// RULES pins (protocols/pair-fable/RULES.md)
...
const MANAGED: Record<string, string> = {
  ...
  '--baselineId': 'not part of the pair-fable workflow; use the raw CLI deliberately if truly needed',
  '--mode': 'orderbook mode is the only supported mode for this protocol',
  '--order': 'replay order is left at the engine default',
  '--time-driven': 'not part of the pair-fable workflow',
  '--realtime': 'not part of the pair-fable workflow',
