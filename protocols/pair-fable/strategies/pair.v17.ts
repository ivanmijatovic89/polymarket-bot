/**
 * pair-fable-v17 — DIRECTIONAL two-sided inventory controller, signal (b):
 * spot-vs-priceToBeat leader (E-042; pair-v17.md — frozen before this file).
 *
 * Delta over pair.v16.ts (v16.2 at d204df35) — ONE substitution:
 *
 *   - LEADER SIGNAL: the book-implied leader (bestBid gap ≥ leadGap) is
 *     replaced by the resolution-relevant price signal: leader = UP when
 *     binance spot − priceToBeat ≥ threshold, DOWN when ≤ −threshold, none
 *     inside the dead zone. Threshold = spotLeadBps basis points of
 *     priceToBeat. Feeds absent (either sub-feed) ⇒ no leader ⇒ neutral.
 *   - Feeds are requested via ExternalFeedsRequestPlugin
 *     (binanceWsSpotPrice + polymarketPriceToBeat); in backtests both are
 *     fulfilled from historical data (aggTrades as-of lookup; Gamma strike
 *     with measured availability latency).
 *
 * EVERYTHING else — signed target T, band guard on e_s, graded lag pricing,
 * FOK completion toward target, tiltUnitMax acquisition-price gate on the
 * tilt component, leadPersistTicks streak (now on the feed-derived leader),
 * VWAP ceiling + capital reservation RAW, pricing grid, cooldowns, TTL,
 * doom backstop, end-of-window cancels, fill-mode tags S/R/C/D — is
 * byte-identical to v16.2. tiltShares = 0 reduces exactly to v15.4 neutral.
...
export const definition: StrategyDefinition<Config> = {
  id: 'pair-fable-v17',
  title: 'pair-fable v17 (directional controller, spot-vs-priceToBeat leader)',
  description:
    'The v16 directional two-sided controller with the leader signal replaced: leader = side favored by binance spot vs the market strike (priceToBeat), declared when the gap ≥ spotLeadBps basis points. The signed error vs the tilt target replaces raw surplus in the band guard, graded lag pricing, leadStop, and FOK completion; tiltUnitMax gates the tilt component of completions by acquisition price. VWAP ceiling and capital reservation stay raw (conservative). tiltShares=0 is exactly v15.4. No sells, no merges; holds to settlement.',
  schema: ConfigSchema,
