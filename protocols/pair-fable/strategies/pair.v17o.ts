/**
 * pair-fable-v17o — state-conditioned maker quote ceiling (pair-v17o.md —
 * design drafted before this file, s34).
 *
 * Delta over pair.v17.ts (at f107234) — ONE substitution + THREE schema adds:
 *
 *   - The maker quote cap pHat gets a completion-deficit PER-SHARE
 *     concession: `pHat = <v17 projection> − oTighten · oDef`, where
 *     oDef = clamp01(1 − matched / (oRefRate · elapsedMin)) after
 *     oGraceMin, else 0, and matched = min(qty.UP, qty.DOWN).
 *     Mechanism prior (pair-v17.md §11/§11.1, run 1008): the ENTIRE S-flow
 *     adverse-selection loss is concentrated in markets whose matched pace
 *     is low by minute 5, and ~40% of it accrues after that flag is up;
 *     on-pace markets' S flow is fair — so the concession is conditioned
 *     on the controller's own completion pace, not the clock (contrast
 *     v17t). Applied after the projection, not through pairTarget (that
 *     would amplify the dose by Qs2/q — defect proven in runs 1015/1016).
 *     oTighten = 0 ⇒ exact v17.
 ...
 * oTighten = 0 reduces exactly to v15.4 neutral. No sells, no merges;
 * holds to settlement.
 */
import type {
  AccountEvent,
...
export const ConfigSchema = z
  .strictObject({
    ...
    /** v17o: per-share maker quote-cap concession in $ at FULL completion deficit — pHat is lowered by oTighten·oDef where oDef = clamp01(1 − matched/(oRefRate·elapsedMin)) after oGraceMin. Applies ONLY to the maker quote cap; pLock/doom stay on base pairTarget. 0 = exact v17. */
    oTighten: z.coerce.number().finite().min(0).max(0.2).default(0),
    /** v17o: reference matched-pair pace in shares/min against which the deficit is measured (30 ⇒ on-pace = 150 matched by minute 5, the §11.1 measured cut). */
...
export const definition: StrategyDefinition<Config> = {
  id: 'pair-fable-v17o',
  title: 'pair-fable v17o (state-conditioned maker quote ceiling)',
  description:
    'pair.v17 with a completion-deficit per-share concession on the maker quote cap: pHat is lowered by oTighten·oDef dollars, oDef = clamp01(1 − matched/(oRefRate·elapsedMin)) after oGraceMin (else 0), matched = min(UP, DOWN) held shares. Applied after the VWAP projection and only to the maker quote price cap (pLock and doom backstop stay on base pairTarget — completions are ~fair per the run-1008 leg-vs-outcome identity). Mechanism: the entire S-flow adverse-selection loss is concentrated in markets whose matched pace is low by minute 5 (pair-v17.md §11.1), so the concession is conditioned on the controller’s own completion pace rather than the clock. oTighten=0 is exactly v17; with tiltShares=0 that is exactly v15.4 neutral. No sells, no merges; holds to settlement.',
  schema: ConfigSchema,
      ...
      if (quotable && bk && obk) {
        ...
        const vOProj =
          D > 0
            ...
            : cost[o] / qty[o]
        // v17o: completion-deficit per-share concession on the maker quote
        // cap (S-quote only; pLock/doom stay on base pairTarget — header).
        ...
        const elapsedMin =
          endMs !== null
            ? Math.min(15, Math.max(0, 15 - (endMs - nowMs) / 60_000))
            : 0
