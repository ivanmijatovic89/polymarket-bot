/**
 * smoke.ts — the mandatory pre-fleet local gate for pair-opus strategies.
 *
 * Usage (from repo root):
 *   tsx protocols/pair-opus/tools/smoke.ts --strategy <id> [--param k=v ...]
 *       [--limit N] [--latest] [--json]
 *
 * What it does:
 *   1. For pair-opus strategies, runs `npm run protocol:check -- pair-opus`
...
const RUN_BACKTEST = path.join(ROOT, 'protocols', 'pair-opus', 'tools', 'run-backtest.ts')
...
// -- 1. protocol:check gate for pair-opus strategy files -------------------
let protocolCheck: 'passed' | 'skipped' = 'skipped'
if (strategy.startsWith('pair-opus-')) {
  console.error('[smoke] running npm run protocol:check -- pair-opus …')
  const r = spawnSync('npm', ['run', 'protocol:check', '--', 'pair-opus'], {
    cwd: ROOT,
    encoding: 'utf8',
  })
  if (r.status !== 0) {
