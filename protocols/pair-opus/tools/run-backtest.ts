/**
 * run-backtest.ts — the canonical pair-opus backtest launcher.
 *
 * Usage (from repo root):
 *   tsx protocols/pair-opus/tools/run-backtest.ts --strategy <id> [options]
 *
 ...
 *      140/20 ms, provenance --protocol pair-opus --model <id>.
 ...
 *      latency (PROPOSALS P-001), and every pair-opus run is latency-pinned
 ...
 *   --label <name>           batchUid prefix for grouping (default 'pf');
 *                            batch_uid becomes '<label>-<utc>-<rand>' — group
 *                            queries use `batch_uid LIKE '<label>-%'`
 *   --model <id>             provenance model override (default claude-fable-5)
...
// RULES pins (protocols/pair-opus/RULES.md)
...
const MANAGED: Record<string, string> = {
  ...
  '--baselineId': 'not part of the pair-opus workflow; use the raw CLI deliberately if truly needed',
  '--mode': 'orderbook mode is the only supported mode for this protocol',
  '--order': 'replay order is left at the engine default',
  '--time-driven': 'not part of the pair-opus workflow',
  '--realtime': 'not part of the pair-opus workflow',
