...
## Run-verified (2026-07-30, run 856: pair-opus-probe-capital-v0, 3 markets, multi-buy both sides, 22 taker fills, winning-side settlement)

- **cost == invested CONFIRMED for no-sell strategies, winning side included**: on all 3 markets `cost` = Σ(buy price·size) + Σ(taker fees) to the cent (e.g. 0.4098×51 + 0.59×15 + 1.11 = 30.86 stored), and `pnl = mergable + remainingWinner − cost` exactly. Settlement never mutates the Portfolio — winning shares do NOT reduce stored basis. [db run 856 | 2026-07-30]
