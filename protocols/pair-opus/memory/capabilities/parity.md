...
| Order status stream | synthetic: MATCHED (sizeMatched 0) at placement, CONFIRMED only on FOK complete fill (GTC/GTD complete fills emit order_done('filled') with NO CONFIRMED — m8), **MINED never**, RETRYING/FAILED never [code BacktestExecution.ts:384,462-477,515-529] | real progression MATCHED→MINED→CONFIRMED; RETRYING/FAILED possible — a MATCHED trade can still fail on-chain [code userWsAccountSource.ts:42-49] |
...
## 5. pair-opus strategy conventions (binding — parity-safe by construction)
...
7. **Stamp `Intent.meta` on every order** (side/price/size/tick context) — the
   ...
   dedup/price-improvement caveats: this note (verified
   run 856).
...
1. **Universe**: full protocol universe (≥ 2026-04-02, all eligible btc 15m
   markets), run via the fleet with provenance (`--protocol pair-opus --model
