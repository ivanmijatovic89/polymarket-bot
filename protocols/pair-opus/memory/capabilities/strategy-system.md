...
`baseline-pair-strategy` (runs 861/862, strategy `pair-opus-v0`): protocol
...
- `protocols/pair-opus/strategies/**` auto-discovered FAIL-SOFT: broken file / wrong prefix ⇒ warn + skip ⇒ symptom is "unknown strategy id" at run time. Ids MUST start `pair-opus-`. Core src/strategies ids win collisions. **Always run `npm run protocol:check -- pair-opus` before pushing** (scoped tsc --noEmit + eslint; exit 0 if no strategies yet). [code src/strategy/protocolStrategyDiscovery.ts:78-197; scripts/protocol-check.mts]
