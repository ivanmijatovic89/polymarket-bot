/**
 * smoke.ts — the mandatory pre-fleet local gate for pair-game-template strategies.
 *
 * Usage (from repo root):
 *   tsx protocols/pair-game-template/tools/smoke.ts --strategy <id> [--param k=v ...]
 *       [--limit N] [--latest] [--json]
 *
 * What it does:
 *   1. For pair-game-template strategies, runs `npm run protocol:check -- pair-game-template`
...
const RUN_BACKTEST = path.join(ROOT, 'protocols', 'pair-game-template', 'tools', 'run-backtest.ts')
...
// -- 1. protocol:check gate for pair-game-template strategy files -------------------
let protocolCheck: 'passed' | 'skipped' = 'skipped'
if (strategy.startsWith('pair-game-template-')) {
  console.error('[smoke] running npm run protocol:check -- pair-game-template …')
  const r = spawnSync('npm', ['run', 'protocol:check', '--', 'pair-game-template'], {
    cwd: ROOT,
    encoding: 'utf8',
  })
  if (r.status !== 0) {
