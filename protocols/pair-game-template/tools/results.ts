 ...
 *   tsx protocols/pair-game-template/tools/results.ts --run 857
 *   tsx protocols/pair-game-template/tools/results.ts --run 856,857 --json
 *   tsx protocols/pair-game-template/tools/results.ts --batch-uid smoke-20260730T194744-4vrhbh
 *   tsx protocols/pair-game-template/tools/results.ts --label smoke --limit 10
 *   tsx protocols/pair-game-template/tools/results.ts --last 5            # recent pair-game-template runs
 *   tsx protocols/pair-game-template/tools/results.ts --run 857 --markets # per-market rows
 *   tsx protocols/pair-game-template/tools/results.ts --run 857 --segments daily
 *
 * Exactly one selector: --run | --batch-uid | --label | --last.
 *
 * Numbers reported:
 *   - headline: the segments kind='all' row (source of truth for run stats)
 *   - capital units: investedTotal (Σcost),
...
try {
  ...
  else runs = await fetchRecentProtocolRuns(conn, 'pair-game-template', o.last!)
