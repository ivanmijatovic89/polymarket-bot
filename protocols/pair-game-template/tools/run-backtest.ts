/**
 * run-backtest.ts — the canonical pair-game-template backtest launcher.
 *
 * Usage (from repo root):
 *   tsx protocols/pair-game-template/tools/run-backtest.ts --strategy <id> [options]
 *
 ...
 *      140/20 ms, provenance --protocol pair-game-template --model <id>.
 ...
 *      latency (PROPOSALS P-001), and every pair-game-template run is latency-pinned
 ...
 *   --label <name>           batchUid prefix for grouping (default 'pgt');
...
// RULES pins (protocols/pair-game-template/RULES.md)
...
const PROTOCOL = 'pair-game-template'
...
const MANAGED: Record<string, string> = {
  ...
  '--baselineId': 'not part of the pair-game-template workflow; use the raw CLI deliberately if truly needed',
  '--mode': 'orderbook mode is the only supported mode for this protocol',
  '--order': 'replay order is left at the engine default',
  '--time-driven': 'not part of the pair-game-template workflow',
  '--realtime': 'not part of the pair-game-template workflow',
