/**
 * smoke.ts — the mandatory pre-fleet local gate for pair-game-opus strategies.
 *
 * Usage (from repo root):
 *   tsx protocols/pair-game-opus/tools/smoke.ts --strategy <id> [--param k=v ...]
 *       [--limit N] [--latest] [--json]
 *
 * What it does:
 *   1. For pair-game-opus strategies, runs `npm run protocol:check -- pair-game-opus`
...
// -- 1. protocol:check gate for pair-game-opus strategy files -------------------
let protocolCheck: 'passed' | 'skipped' = 'skipped'
if (strategy.startsWith('pair-game-opus-')) {
  console.error('[smoke] running npm run protocol:check -- pair-game-opus …')
  const r = spawnSync('npm', ['run', 'protocol:check', '--', 'pair-game-opus'], {
    cwd: ROOT,
    encoding: 'utf8',
  })
  if (r.status !== 0) {
