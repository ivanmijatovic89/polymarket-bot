...
/**
 * SpikeMomentum.v1 — "follow the jerk, ride the continuation".
 *
 * Born from the OverreactionSnap.v1 result: fading a fast UP-mid spike won
 * only 27.6% of markets over the last 1000 btc 15m markets. A sub-50% win rate
 * on a directional bet is a *flipped signal*: spikes mostly CONTINUE, they do
 * not revert. So this strategy is OverreactionSnap with exactly ONE thing
 * changed — the entry side. We buy the side that is spiking, not the opposite.
 *
 * Everything else (spike detection, exit on held-token mid rising by
 * `takeProfit`, stop on `stopLoss`, `maxHoldSec`, late-window bailout, the
 * `minSecondsLeft` open guard) is identical to OverreactionSnap.v1 so the two
 * form a clean A/B test of fade-vs-follow.
 *
 *  - Spike UP in UP-mid  -> buy UP   (ride the move up).
 *  - Spike DOWN in UP-mid -> buy DOWN.
 *
 * Order-book only; no external feeds.
 *
 * LIVE CAVEAT: buy-then-sell. In live trading you must wait for MINED before
 * selling shares you just bought (see CLAUDE.md). Backtest books on fill.
 */
...
export const ConfigSchema = z.strictObject({
  ...
  /** Profit target (held-token mid gain = continuation) that closes a win. */
...
export const definition: StrategyDefinition<Config> = {
  id: 'SpikeMomentum.v1',
  title: 'Spike Momentum v1',
  description:
    'Follows a fast UP-mid spike (buys the spiking side) and exits on continuation/stop/timeout. ' +
    'Direction-flipped sibling of OverreactionSnap.v1. Order-book only; refuses to open near expiry.',
  schema: ConfigSchema,
...
export function createStrategy(cfg: Config): Strategy {
  ...
  const onMarketTick = (
    ...
    // MOMENTUM: spike UP -> buy UP (follow); spike DOWN -> buy DOWN.
    // (This is the ONLY line that differs from OverreactionSnap.v1.)
    ...
    return [
      {
        ...
        size: cfg.size,
        orderType: 'FOK',
        reason: delta > 0 ? 'follow_spike_up_buy_up' : 'follow_spike_down_buy_down',
      },
    ]
  }
