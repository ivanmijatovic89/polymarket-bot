...
export const definition: StrategyDefinition<Config> = {
  id: 'SplitSellRedeem.v5.3-technical-indicators',
  title: 'Split + sell with dwell + time filters v5.3 + technical indicators',
  description:
    'Splits collateral into UP+DOWN (full set). Tracks dwell time per side and trades only within an allowed time window. Uses technical indicators to make trading decisions.',
  schema: ConfigSchema,
...
export function createStrategy(cfg: Config): {
  ...
  const name = 'SplitSellRedeem.v5.3-technical-indicators'
