...
export const definition: StrategyDefinition<Config> = {
  id: 'SplitSellRedeem.v5.2-netChange',
  title: 'Split + sell with dwell + time filters v5.2-netChange',
  description:
...
export function createStrategy(cfg: Config): {
  ...
  const name = 'SplitSellRedeem.v5.2-netChange'
  ...
  const plugins: Plugin[] = [timeWindowGatePlugin, dwellGatePlugin, externalFeedsPlugin, timeWindowVolatilityPlugin]
  ...
  const onMarketTick = (tick: MarketTick, portfolio: PortfolioSnapshot, ctx?: StrategyContext): Intent[] => {
    ...
    const netChange = volByAsset?.['45s']?.netChange ?? null

    if(netChange === null) return []

    if(netChange > cfg.netChangeThreshold) return []
