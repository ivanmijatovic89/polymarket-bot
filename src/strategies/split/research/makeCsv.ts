...
const inputPath = path.join(
  __dirname,
  "results-SplitSellRedeem.v5.1-research-metrics.ts",
);
...
const main = () => {
  ...
  for (const market of markets) {
    ...
    const metricMap = new Map(
      windowsMetrics.map((metric) => [metric.window, metric]),
    );
    ...
    netChangeJson.push({
      ...
      ...Object.fromEntries(
        WINDOWS.map((window) => [
          window,
          metricMap.get(window)?.netChange ?? null,
        ]),
      ),
    });
    highLowJson.push({
      ...
      ...Object.fromEntries(
        WINDOWS.map((window) => [
          window,
          metricMap.get(window)?.highLowRange ?? null,
        ]),
      ),
    });
  }
  ...
  fs.writeFileSync(
    highLowRangeJsonPath,
    JSON.stringify(highLowJson, null, 2),
  );
