...
async function run() {
  ...
  console.log(
    `[fetch] 1h ${SYMBOL} ${new Date(start1h).toISOString()} -> ${new Date(endMs).toISOString()}`,
  )
  ...
  console.log(
    `[fetch] 15m ${SYMBOL} ${new Date(start15m).toISOString()} -> ${new Date(endMs).toISOString()}`,
  )
  const klines15m = await fetchKlines({
    symbol: SYMBOL,
    interval: '15m',
    startTimeMs: start15m,
    endTimeMs: endMs,
  })
  ...
  const csvPath = outPath.toLowerCase().endsWith('.json')
