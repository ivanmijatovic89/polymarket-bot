...
export const ConfigSchema = z.strictObject({
  ...
  takeProfit: z.coerce.number().finite().positive().max(0.49).default(0.04),
})
...
export const definition: StrategyDefinition<Config> = {
  id: 'maker-favorite.012-cancel-take-profit',
  title: 'Maker favorite cancel take profit',
  description:
    'Places one delayed favorite maker bid, cancels if it weakens before fill, then rests a maker take-profit sell for any filled position.',
  schema: ConfigSchema,
...
export function createStrategy(cfg: Config): { strategy: Strategy } {
  const name = 'maker-favorite.012-cancel-take-profit'
  ...
  const onMarketTick = (
    ...
    if (boughtAssetId && !sellPlaced) {
      ...
      if (
        ...
        const sellPrice = safeProbabilityPrice(round2(entry + cfg.takeProfit))
        if (sellPrice >= 0.01 && sellPrice <= 0.99) {
          sellPlaced = true
          return [
            {
              kind: 'place_limit',
              clientOrderId: `${name}:${marketKey ?? 'mkt'}:sell`,
              assetId: boughtAssetId,
              side: 'SELL',
              price: sellPrice,
              size: qty,
              orderType: 'GTC',
              reason: `take_profit entry=${entry.toFixed(4)} sell=${sellPrice.toFixed(2)}`,
            },
          ]
        }
      }
