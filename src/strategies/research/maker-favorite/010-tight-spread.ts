...
export const ConfigSchema = z.strictObject({
  ...
  maxFavSpread: z.coerce.number().finite().positive().max(0.99).default(0.06),
})
...
export const definition: StrategyDefinition<Config> = {
  id: 'maker-favorite.010-tight-spread',
  title: 'Maker favorite tight spread',
  description:
    'Places one delayed maker bid on a stronger favorite only when its book spread is tight, then cancels if that side weakens before fill.',
  schema: ConfigSchema,
...
export function createStrategy(cfg: Config): { strategy: Strategy } {
  const name = 'maker-favorite.010-tight-spread'
  ...
  const onMarketTick = (
    ...
    const favSpread = fav.bestAsk - fav.bestBid

    if (favMid < cfg.favThreshold || favSpread > cfg.maxFavSpread) {
      orderPlaced = true
      return []
    }
    ...
    return [
      {
        ...
        size: cfg.size,
        orderType: 'GTC',
        reason: `tight-spread favorite ${side} elapsed=${elapsedSec.toFixed(1)}s mid=${favMid.toFixed(4)} spread=${favSpread.toFixed(4)} bid=${bidPrice.toFixed(2)}`,
      },
    ]
  }
