...
export const definition: StrategyDefinition<Config> = {
  id: 'maker-favorite.000-baseline',
  title: 'Maker favorite baseline',
  description:
    'Places one resting maker bid below mid on the currently higher-mid leg, then holds any fill to resolution.',
  schema: ConfigSchema,
...
export function createStrategy(cfg: Config): { strategy: Strategy } {
  const name = 'maker-favorite.000-baseline'
    ...
    return [
      {
        ...
        size: cfg.size,
        orderType: 'GTC',
        reason: `favorite ${side} mid=${favMid.toFixed(4)} bid=${bidPrice.toFixed(2)}`,
      },
    ]
  }
