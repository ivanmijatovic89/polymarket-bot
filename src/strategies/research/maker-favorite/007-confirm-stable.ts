...
export const ConfigSchema = z.strictObject({
  ...
  confirmSec: z.coerce.number().finite().nonnegative().default(30),
  maxWeakening: z.coerce.number().finite().nonnegative().max(0.49).default(0.01),
})
...
export const definition: StrategyDefinition<Config> = {
  id: 'maker-favorite.007-confirm-stable',
  title: 'Maker favorite confirm stable',
  description:
    'Arms on a delayed stronger favorite, waits briefly, and places one maker bid only if the same side remains stable.',
  schema: ConfigSchema,
...
export function createStrategy(cfg: Config): { strategy: Strategy } {
  const name = 'maker-favorite.007-confirm-stable'
  ...
  const onMarketTick = (
    ...
    if (
      favAssetId !== armed.assetId ||
      favMid < cfg.favThreshold ||
      favMid < armed.mid - cfg.maxWeakening
    ) {
      orderPlaced = true
      return []
    }
    ...
    return [
      {
        ...
        size: cfg.size,
        orderType: 'GTC',
        reason: `confirmed favorite ${armed.side} elapsed=${elapsedSec.toFixed(1)}s armedMid=${armed.mid.toFixed(4)} mid=${favMid.toFixed(4)} bid=${bidPrice.toFixed(2)}`,
      },
    ]
  }
