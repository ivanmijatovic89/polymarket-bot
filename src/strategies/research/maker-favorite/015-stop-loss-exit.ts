...
export const ConfigSchema = z.strictObject({
  ...
  stopLoss: z.coerce.number().finite().positive().max(0.49).default(0.04),
})
...
export const definition: StrategyDefinition<Config> = {
  id: 'maker-favorite.015-stop-loss-exit',
  title: 'Maker favorite stop-loss exit',
  description:
    'Places one delayed favorite maker bid, then exits filled inventory with a FOK sell if the side weakens past a stop-loss threshold.',
  schema: ConfigSchema,
...
function hasActiveSell(portfolio: PortfolioSnapshot, assetId: string): boolean {
  for (const order of Object.values(portfolio.openOrdersByClientId)) {
    if (order.assetId !== assetId) continue
    if (order.side !== 'SELL') continue
    if (ACTIVE_STATES.has(order.state)) return true
  }
  return false
}

export function createStrategy(cfg: Config): { strategy: Strategy } {
  const name = 'maker-favorite.015-stop-loss-exit'
  ...
  const onMarketTick = (
    ...
    if (boughtAssetId && !stopPlaced) {
      const pos = portfolio.positionsByAssetId[boughtAssetId]
      const qty = pos?.qty ?? 0
      const entry = pos?.avgEntryPrice ?? buyPrice
      ...
      if (
        ...
        validBook(book) &&
        book.mid <= entry - cfg.stopLoss &&
        !hasActiveSell(portfolio, boughtAssetId)
      ) {
        const sellPrice = safeProbabilityPrice(round2(book.bestBid))
        if (sellPrice >= 0.01 && sellPrice <= 0.99) {
          stopPlaced = true
          return [
            {
              kind: 'place_limit',
              clientOrderId: `${name}:${marketKey ?? 'mkt'}:stop`,
              assetId: boughtAssetId,
              side: 'SELL',
              price: sellPrice,
              size: qty,
              orderType: 'FOK',
              reason: `stop_loss entry=${entry.toFixed(4)} mid=${book.mid.toFixed(4)} sell=${sellPrice.toFixed(2)}`,
            },
          ]
        }
      }
