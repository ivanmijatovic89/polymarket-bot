...
A wallet's activity was filtered against the catalog **as it existed at sync time**. If you later lower the backfill floor or add an older symbol/timeframe, markets the wallet traded in get cataloged *after* it was synced — and their activity (fetched but dropped as not-in-catalog originally) now sits behind the wallet's cursor, so a normal `cursor − 1h` refresh never re-reads it. `sync-activity` detects this automatically: at start it rebases affected wallets' cursors back to the earliest newly-covered market and re-queues them. Detection uses `polymarket_markets.synced_at` (catalog-insert time, which the catalog upsert never overwrites) — a wallet is affected when it participated (trade **or** position) in a market with `synced_at > wallet.activity_synced_at` whose `market_start_ms` predates the wallet's refresh window. A cheap pre-check on the small markets table skips the trades/positions join entirely in normal operation, so this costs nothing until coverage actually expands. The re-scan is idempotent (`dedup_key`) and concurrent-safe (the `= 'done'` status guard).
:::
