...
7. **If the mission touches a Git repository, own the Git discipline.** The runtime never runs Git. Instruct the agent to work on a named branch and commit at the end of every session, so an interrupted session loses at most one unit of work.
