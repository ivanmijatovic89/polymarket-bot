...
| Run refuses to start: "workspace … overlaps the runtime log root" | The launch anchors that pin each run's immutable settings live under the log root, so a session that could write there could disarm its own pin. A mission whose workspace contains the daemon's `logs/global-runtime` (for example a workspace that IS the engine repo) is refused — point `GLOBAL_RUNTIME_LOG_DIR` outside the workspace and start it again. |
| Run refuses to start: "immutable … changed since it was first launched" | The run's `provider`/`model`/`accessMode`/`authHome`/`workspacePath`/`sandboxSettingsPath` differs from what it was first launched with. These are immutable; a difference means the row was edited outside Mission Control. Verify the configuration is legitimate, then delete that run's file under `logs/global-runtime/anchors/` to re-pin, or create a new run. |
| Run refuses to start: "launch anchor … is unreadable" | The anchor file is missing (for a run that already started), corrupt, or not a launch spec. Same recovery: confirm the configuration, then remove the anchor file so the next start re-pins it. |
