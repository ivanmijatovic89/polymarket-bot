...
| Variable                                   | Type                             | Default       | Description                                                                                                                                                                                                                                   |
| ------------------------------------------ | -------------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DRY_RUN` | `boolean` | `true` | Only the value `false` (case-insensitive) enables real execution; unset or any other value keeps dry-run enabled. Dry-run synthesizes acceptance without fills or post-only crossing checks. See [live compatibility](../engine/live-execution.md). |
...
| `MAX_EVENTS_PER_DRAIN` | `positive safe integer` | `4200` | Account events processed per drain cycle, shared by live trading and backtests. Set the same value on every fleet worker; invalid values fail configuration validation. |
...
| Variable | Type | Default | Description |
| --- | --- | --- | --- |
| `GLOBAL_RUNTIME_HOST` | `string` | `127.0.0.1` | Control API bind address. Set the machine's Tailscale IP to make the daemon reachable from Mission Control on another machine — the daemon refuses a non-loopback bind unless `GLOBAL_RUNTIME_TOKEN` is set. |
| `GLOBAL_RUNTIME_PORT` | `integer` | `3053` | Control API port. |
