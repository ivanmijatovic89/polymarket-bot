// Glitch Foundry mid-window checkpoint extractor (holdout set, round 6 interleave).
// Derived from extract_endgame.cjs with three changes ONLY:
//   - Checkpoints at t in {300, 450, 600, 690}.
//   - Fast start anchors at the last `book` snapshot per asset with
//     ts <= epoch+300s (min across assets); full-file fallback as before.
//   - Replay cutoff at epoch+690s (nothing after the last checkpoint is needed).
// Everything else (book semantics, self-check, one-sided books kept, output
// format, resumability) is identical to extract_endgame.cjs / ENGINE.md:
// book = full replacement; price_change size 0 removes, non-zero upserts;
// side 0=bid, 1=ask. Checkpoint at T = state after all events with
// ts_local_ms <= epoch*1000+T*1000 in ingest_seq order.
//
// Usage: node extract_midwindow.cjs <manifest.csv> <outDir> [batchSize=100]
...
async function processFile(conn, ep) {
  ...
  const cutoff = winStart + CUTOFF_S * 1000;
  const anchor = winStart + ANCHOR_S * 1000;

  // Fast start: last book snapshot per asset at ts <= t300.
