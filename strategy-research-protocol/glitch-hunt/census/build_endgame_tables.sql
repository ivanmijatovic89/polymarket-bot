-- Endgame holdout tables (Foundry round 3 interleave).
-- Run from strategy-research-protocol/glitch-hunt/ :
--   nice -n 19 duckdb < census/build_endgame_tables.sql
SET threads TO 2;
SET memory_limit='3GB';

-- 1. Canonical parquet: all endgame checkpoints with derived columns.
--    One-sided/empty books are KEPT (bid/ask recorded independently).
CREATE TEMP TABLE eg AS
SELECT
...
FROM read_csv_auto('census/endgame/checkpoints/batch-*.csv', header=true, union_by_name=true);

COPY (SELECT * FROM eg) TO 'census/endgame_checkpoints.parquet' (FORMAT PARQUET);

-- 2. Taxonomy: book-state counts by t, per month and ALL (up book; down book mirrors).
COPY (
  WITH base AS (
    SELECT month, t_sec, up_state, up_won FROM eg
    UNION ALL
    SELECT 'ALL', t_sec, up_state, up_won FROM eg
  )
...
) TO 'census/endgame_taxonomy.csv' (HEADER);

-- 3. Two-sided mid calibration (comparable to old endgame.csv, now holdout mass).
COPY (
  WITH base AS (
    SELECT month, t_sec, band_c, up_won, spread FROM eg WHERE band_c IS NOT NULL
    UNION ALL
    SELECT 'ALL', t_sec, band_c, up_won, spread FROM eg WHERE band_c IS NOT NULL
  )
  ...
  FROM base GROUP BY 1,2,3 ORDER BY 1,2,3
) TO 'census/endgame_calibration_mid.csv' (HEADER);

-- 4. Takeable-side calibration: per token, rows where that token's ASK exists,
--    split two_sided vs ask_only. p_win is P(that token wins) vs its ask price.
COPY (
  WITH tok AS (
...
) TO 'census/endgame_calibration_takeable.csv' (HEADER);

-- 5. Bid-side (sellable/short-via-other-token) calibration, same shape.
COPY (
  WITH tok AS (
    SELECT month, t_sec, 'up' AS token,
  ...
  FROM base GROUP BY 1,2,3,4,5 ORDER BY 1,2,3,4,5
) TO 'census/endgame_calibration_bid.csv' (HEADER);

-- 6. Friction map extension: endgame cells, same shape as census friction_map.csv
--    (up book), appended as source='endgame_holdout', plus per-side extras.
COPY (
  SELECT 'endgame_holdout' AS source, t_sec, band_c, count(*) AS n,
  ...
  FROM eg WHERE band_c IS NOT NULL
  GROUP BY t_sec, band_c ORDER BY t_sec, band_c
) TO 'census/friction_map_endgame.csv' (HEADER);
...
COPY (
  ...
  FROM read_csv_auto('census/endgame/selfcheck.csv', header=true) s
...
) TO 'census/endgame_selfcheck_by_month.csv' (HEADER);
